...
export async function POST(request: Request) {
  try {
    ...
    if (slotError) {
      return new Response(
        JSON.stringify({ error: `Error fetching slot data: ${slotError.message}` }),
        { status: 500 }
      );
    }
    ...
    if (updateError) {
      return new Response(
        JSON.stringify({ error: `Error updating slot: ${updateError.message}` }),
        { status: 500 }
      );
    }
    ...
    if (bookingError) {
      return new Response(
        JSON.stringify({ error: `Error logging the booking: ${bookingError.message}` }),
        { status: 500 }
      );
    }

    return new Response(
      JSON.stringify({ message: 'Booking request sent successfully', data: updatedSlot }),
      { status: 200 }
    );
  } catch (error) {
    return new Response(
